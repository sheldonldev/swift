...
class ArgumentsBase:
    ...
    def _check_path(
        ...
        check_exist_path_set: Optional[Set[str]],
    ) -> Union[str, List[str]]:
    ...
    def select_dtype(
        ...
        if not is_torch_cuda_available() and not is_torch_npu_available():
            ...
            assert (
                self.dtype != "fp16"
            ), "The CPU does not support matrix multiplication with FP16."
            if self.dtype == "fp32":
    ...
    def handle_compatibility(self: Union["SftArguments", "InferArguments"]) -> None:
        ...
        for _name, _mapping in [
            ["template_type", template_type_mapping],
            ["model_type", model_type_mapping],
        ]:
    ...
    def _handle_dataset_sample(self):
        ...
        if (
            len(self.dataset) == 1
            and "#" not in self.dataset[0]
            and self.train_dataset_sample >= 0
        ):
    ...
    def _handle_dataset_compat(
        ...
        if train_dataset is not None and self.train_dataset_sample >= 0:
            train_dataset_sample = min(
                self.train_dataset_sample, train_dataset.shape[0]
            )
            ...
            if val_dataset_sample is None:
                val_dataset_sample = max(
                    int(train_dataset_sample * self.dataset_test_ratio), 1
                )
        if (
            val_dataset is not None
            and val_dataset_sample is not None
            and val_dataset_sample >= 0
        ):
        ...
        if (
            ...
            or not hasattr(self, "train_dataset_mix_ratio")
            or self.train_dataset_mix_ratio <= 0
        ...
        if len(mixed_dataset) < mix_dataset_sample:
            logger.warn(
                f"The length of dataset used for mixin: {self.train_dataset_mix_ds} are "
            ...
            mixed_dataset = sample_dataset(
                mixed_dataset, mix_dataset_sample, random_state
            )
    ...
    def set_model_type(self: Union["SftArguments", "InferArguments"]) -> None:
        ...
        if self.model_id_or_path is not None:
            ...
            if model_id_or_path_lower not in model_mapping_reversed:
                if (
                    isinstance(self, InferArguments)
        ...
        if self.model_type is None:
            ...
            raise ValueError(
                f"model_type: '{self.model_type}' is not registered. " + error_msg
            )
        model_info = MODEL_MAPPING[self.model_type]
        use_hf = strtobool(os.environ.get("USE_HF", "False"))
        ...
        if self.model_id_or_path is None:
            self.model_id_or_path = (
                model_info["hf_model_id"] if use_hf else model_info["model_id_or_path"]
            )
...
class SftArguments(ArgumentsBase):
    ...
    model_layer_cls_name: Optional[str] = field(
        default=None,
        metadata={
            "help": "Decoder Class name of model, e.g. 'QWenBlock' for QWen, 'LlamaDecoderLayer' for LLama"
        },
    )

    sft_type: Literal[
        "lora", "full", "longlora", "adalora", "ia3", "llamapro", "adapter"
    ] = "lora"
    freeze_parameters: float = 0.0  # 0 ~ 1
    additional_trainable_parameters: List[str] = field(default_factory=list)
    tuner_backend: Literal["swift", "peft", "unsloth"] = "peft"
    template_type: str = field(
        default="AUTO",
        metadata={
            "help": f"template_type choices: {list(TEMPLATE_MAPPING.keys()) + ['AUTO']}"
        },
    )
    ...
    hub_token: Optional[str] = field(
        default=None,
        metadata={
            "help": "SDK token can be found in https://modelscope.cn/my/myaccesstoken"
        },
    )
    hub_private_repo: bool = False
    push_hub_strategy: Literal[
    ...
    test_oom_error: bool = field(
        default=False,
        metadata={
            "help": "If set to True, the train_dataset will be sorted in descending order based on max_length, "
    ...
    train_dataset_mix_ratio: float = 0.0
    ...
    def __post_init__(self) -> None:
        ...
        if self.val_dataset is not None:
            self.dataset_test_ratio = (
                0.0 if self.val_dataset is not None else self.dataset_test_ratio
            )
        ...
        if (
            self.use_self_cognition
            and self.sft_type == "lora"
            and not self.lora_use_all
        ):
            logger.warning(
        ...
        if is_adapter(self.sft_type):
            ...
            assert (
                self.quantization_bit == 0
            ), "Full parameter fine-tuning does not support quantization."
        ...
        if self.quantization_bit > 0 and self.quant_method is None:
            if self.quantization_bit == 4 or self.quantization_bit == 8:
                logger.info(
                    "Since you have specified quantization_bit as greater than 0\
        ...
        self.bnb_4bit_compute_dtype, self.load_in_4bit, self.load_in_8bit = (
            self.select_bnb()
        )

        if self.neftune_backend is None:
            self.neftune_backend = (
                "swift"
...
class InferArguments(ArgumentsBase):
    ...
    def __post_init__(self) -> None:
        ...
        if self.load_args_from_ckpt_dir:
            ...
            assert (
                self.load_dataset_config is False
            ), "You need to first set `--load_args_from_ckpt_dir true`."
        self._handle_dataset_sample()
        self.handle_custom_register()
        self.handle_custom_dataset_info()
        self.set_model_type()
    ...
    def handle_infer_backend(self):
        ...
        if (
            self.infer_backend == "vllm"
            and self.vllm_enable_lora
            or self.infer_backend == "pt"
            and isinstance(self, DeployArguments)
            and self.sft_type == "lora"
        ):
        ...
        if self.quant_output_dir is None:
            if self.ckpt_dir is None:
                self.quant_output_dir = (
                    f"{self.model_type}-{self.quant_method}-int{self.quant_bits}"
                )
            else:
                ckpt_dir, ckpt_name = os.path.split(self.ckpt_dir)
                self.quant_output_dir = os.path.join(
                    ckpt_dir, f"{ckpt_name}-{self.quant_method}-int{self.quant_bits}"
                )
...
class RomeArguments(InferArguments):
    rome_request_file: str = field(
        default=None,
        metadata={
            "help": "The rome request file, please check the documentation "
            "to get the format"
        },
    )
